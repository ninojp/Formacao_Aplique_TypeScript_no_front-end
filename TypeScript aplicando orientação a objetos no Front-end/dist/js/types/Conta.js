...
export class Conta {
    ...
    getGruposTransacaoes() {
        ...
        let listaTransacoes = structuredClone(this.transacoes);
        if (typeof listaTransacoes === 'string') {
            try {
                listaTransacoes = JSON.parse(listaTransacoes, (key, value) => key === 'data' ? new Date(value) : value);
            }
            catch (e) {
        ...
        if (Array.isArray(listaTransacoes)) {
            listaTransacoes = listaTransacoes.map((t) => ({
                ...t,
